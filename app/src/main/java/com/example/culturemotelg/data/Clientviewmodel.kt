...
import com.example.culturemotelg.models.Client
import com.example.culturemotelg.navigation.ROUTE_CULTUREROOMS
...
class ClientViewModel(): ViewModel() {
    ...
    fun saveClient(firstname: String, lastname: String, gender: String, age: String, phonenumber: String, navController: NavController,
                   context: Context) {
        val id = System.currentTimeMillis().toString()
        val dbRef = FirebaseDatabase.getInstance().getReference("Client/$id")

        val clientData = Client("", firstname, lastname, gender, phonenumber, age, id)

        dbRef.setValue(clientData)
            .addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    showToast("Client added successfully", context)
                    navController.navigate(ROUTE_CULTUREROOMS)
                } else {
    ...
    fun viewClients(client: MutableState<Client>, clients: SnapshotStateList<Client>, context: Context):
    ...
    fun updateClient(context: Context, navController: NavController, firstname: String, lastname: String,
